...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnMaxSalary()
    {
        ...
        decimal? maxSalary = emps.Select(x => x.Sal).Max();
    ...
    {
        var emps = Database.GetEmps();
        decimal? minSalary = emps.Where(x => x.DeptNo == 30).Select(x => x.Sal).Min();
    ...
    public void ShouldReturnFirstTwoHiredEmployees()
    {
        ...
        var firstTwo =  emps.OrderBy(x => x.HireDate).Take(2).ToList();
    ...
    {
        var emps = Database.GetEmps();
        var jobs = emps.Select(x => x.Job).Distinct(); 
    ...
    {
        var emps = Database.GetEmps();
        var withMgr = emps.Where(x => !x.Mgr.Equals(null)); 
    ...
    {
        var emps = Database.GetEmps();
        var result = emps.TrueForAll(x => x.Sal > 500);
        Assert.True(result);
    }
    ...
    {
        var emps = Database.GetEmps();
        var result = emps.Any(x => x.Comm > 400);
        Assert.True(result);
    }
    ...
    {
        var emps = Database.GetEmps();
        var result = from e1 in emps from e2 in emps where e1.Mgr==e2.EmpNo select new {
            Employee = e1.EName,
            Manager = e2.EName
        };
    ...
    {
        var emps = Database.GetEmps();
        var result =  from e in emps select new {e.EName, Total = e.Sal + (e.Comm.Equals(null) ? 0 : e.Comm)};
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = from e in emps
            ...
            from gr in grades
            where e.Sal >= gr.Losal && e.Sal <= gr.Hisal
            select new {e.EName, d.DName, gr.Grade};
