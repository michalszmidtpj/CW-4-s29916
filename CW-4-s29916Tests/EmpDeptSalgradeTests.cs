...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnAllSalesmen()
    {
        ...
        List<Emp> result = emps.Where(x => x.Job.Equals("SALESMAN")).ToList(); 
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(x => x.DeptNo == 30).OrderBy(x => -x.Sal).ToList(); 
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = (
            ...
            where d.Loc == "CHICAGO" select e).ToList(); 
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = 
            from e in emps
            from de in depts 
            where e.DeptNo == de.DeptNo select new {de.DName, e.EName}; 
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = from e in emps group e by e.DeptNo into tmp select new
        {
            Count = tmp.Count(),
            DeptNo = tmp.Key
        }; 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(x => !x.Comm.Equals(null)); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = 
            from e in emps 
            from sl in grades
            where e.Sal >= sl.Losal && e.Sal <= sl.Hisal select new
            {
                e.EName,
                sl.Grade
            };
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = 
            from e in emps 
            group emps by e.DeptNo 
            into tmp select new
        {
            DeptNo = tmp.Key,
            AvgSal = emps.Average(x => x.Sal)
        }; 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = (
            from e1 in emps
            where e1.Sal > emps.Where(x => x.DeptNo == e1.DeptNo).Average(x => x.Sal)
            select e1.EName);
